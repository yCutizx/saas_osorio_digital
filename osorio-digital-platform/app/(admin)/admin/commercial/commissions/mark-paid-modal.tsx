...
import { markCommissionAsPaidAction } from '@/app/actions/commercial'
...
export function MarkPaidModal({ commission }: Props) {
  const [open, setOpen]    = useState(false)
  const [paidAt, setPaidAt]      = useState(todayBRT())
  const [paidAmount, setPaidAmount] = useState(commission.commission_amount.toFixed(2).replace('.', ','))
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startSubmit(async () => {
      const r = await markCommissionAsPaidAction(commission.id, {
        paid_at:        paidAt,
  ...
  const refLabel = format(parseISO(commission.reference_month), 'MM/yyyy', { locale: ptBR })

  return (
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger className="inline-flex items-center gap-1.5 bg-[#EACE00] text-black text-xs font-semibold px-3 py-1.5 rounded-lg hover:bg-[#f5d800] transition-colors">
        <CheckCircle2 className="h-3.5 w-3.5" />
        Marcar paga
      </SheetTrigger>
      <SheetContent side="right" className="w-full sm:max-w-md bg-[#0A0A0A] border-l border-[#222] overflow-y-auto">
        <SheetHeader>
          <SheetTitle className="text-white">Marcar comissão como paga</SheetTitle>
        </SheetHeader>

        <div className="mt-4 rounded-lg bg-[#111] border border-[#222] p-3 text-sm space-y-1">
          <p className="text-[#F5F5F0] font-semibold">{commission.seller_name}</p>
          <p className="text-[#888] text-xs">Cliente {commission.client_name} · Ref. {refLabel}</p>
          <p className="text-[#EACE00] font-bold tabular-nums">{formatBRL(commission.commission_amount)}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="mt-5 space-y-4">
          ...
          <Field
            label="Valor pago (R$)"
            htmlFor="paid-amount"
            hint="Pré-preenchido com o valor da comissão. Edite se houver divergência."
          >
            <input
              id="paid-amount"
              type="text"
