...
import { AlertCircle, Send, FileEdit, Upload, X, FileText, Image as ImageIcon, ExternalLink } from 'lucide-react'
...
function FileReplaceField({
  name, label, accept, currentUrl, icon: Icon,
}: {
  name: string; label: string; accept: string; currentUrl: string | null; icon: React.ElementType
}) {
    ...
    <div className="space-y-1.5">
      ...
      {currentUrl && !file && (
        <a href={currentUrl} target="_blank" rel="noopener noreferrer"
          className="inline-flex items-center gap-1.5 text-xs text-[#EACE00]/70 hover:text-[#EACE00] transition-colors mb-1">
        ...
        <div className="flex-1 min-w-0">
          {file ? (
            ...
            <p className="text-sm text-white/30">
              {currentUrl ? 'Clique para substituir...' : 'Clique para selecionar...'}
            </p>
          )}
        </div>
        {file ? (
...
export function EditInsightForm({ insight, clients }: Props) {
  ...
  return (
    <form action={action} className="space-y-6">
      ...
      <input ref={statusRef} type="hidden" name="status" defaultValue={insight.published ? 'published' : 'draft'} />
      ...
      <div className="space-y-1.5">
        ...
        <input id="title" name="title" required defaultValue={insight.title}
          placeholder="Título do insight" className={INPUT_CLS} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="space-y-1.5">
          ...
          <select id="type" name="type" defaultValue={insight.type ?? ''} className={SELECT_CLS}>
        ...
        <div className="space-y-1.5">
          ...
          <select id="client_id" name="client_id" defaultValue={insight.client_id ?? ''} className={SELECT_CLS}>
      ...
      <div className="space-y-1.5">
        ...
        <input id="tags_raw" name="tags_raw"
          ...
          className={INPUT_CLS} />
        {previewTags && <p className="text-[#EACE00]/60 text-xs">{previewTags}</p>}
      </div>
