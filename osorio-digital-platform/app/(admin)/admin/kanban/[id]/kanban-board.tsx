...
function SortableCard({ card, onOpen, isDragging, isContent, disabled }: {
  card: KanbanCard
    ...
    <div ref={colRef} style={colStyle}
      ...
      <div className="flex items-center gap-2 px-3 py-3 border-b border-[#222]">
        ...
        {renaming ? (
          ...
          <span className="flex-1 text-sm font-semibold text-white cursor-default truncate" onDoubleClick={() => setRenaming(true)}>
            {col.label}
          </span>
        )}
        ...
        <SortableContext items={cards.map((c) => c.id)} strategy={verticalListSortingStrategy}>
          {cards.map((card) => (
            <SortableCard key={card.id} card={card} onOpen={onOpen}
              isDragging={activeCardId === card.id} isContent={isContent} disabled={isDraggingColumn} />
          ))}
        </SortableContext>
        <button
          type="button"
  ...
  if (!open) {
    return (
      <button onClick={() => setOpen(true)}
        ...
        <Plus className="h-4 w-4" />
        <span className="text-sm">Adicionar lista</span>
      </button>
    )
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="bg-[#111] border border-[#222] rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form action={dispatch} className="p-5 space-y-4">
          ...
          <div>
            <label className="text-xs text-white/50 mb-1 block">{isContent ? 'Descrição / Briefing' : 'Descrição'}</label>
          ...
          <div>
            ...
            <input name="tags_raw" defaultValue={card?.tags?.join(', ') ?? ''} placeholder="ex: urgente, revisão"
              className="w-full bg-[#0a0a0a] border border-[#333] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#EACE00]" />
          </div>
...
export function KanbanBoard({ board, initialCards, members, boardMembers, clients, currentUserId, userRole }: Props) {
  ...
  const [deleteTarget, setDeleteTarget]     = useState<{ type: 'card'; card: KanbanCard } | null>(null)
