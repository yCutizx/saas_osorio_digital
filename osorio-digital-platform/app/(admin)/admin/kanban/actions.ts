...
export async function assignCardAction(
  ...
  // Permissão: admin global, criador, ou membro do board
  if (ctx.role !== 'admin') {
  ...
  // Se atribuindo a alguém, validar que é membro do board
  if (assigneeId) {
  ...
  if (error) {
    console.error('[assignCardAction] erro:', error.message)
