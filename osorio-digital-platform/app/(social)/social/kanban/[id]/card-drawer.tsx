        ...
        <div className="flex items-start gap-3 px-5 py-4 border-b border-[#222] shrink-0">
          <div className="flex-1 min-w-0">
            {editingTitle ? (
              ...
              <h2 className="text-white font-semibold text-base cursor-pointer hover:text-white/80 transition-colors leading-snug"
          ...
          <div className="flex items-center gap-1 shrink-0">
            <button onClick={() => { onArchived(); archiveCardAction(card.id, boardId) }}
              className="p-1.5 rounded-lg text-white/30 hover:text-yellow-400 hover:bg-white/5 transition-colors" title="Arquivar">
              <Archive className="h-4 w-4" />
            </button>
            <button onClick={onDelete}
              className="p-1.5 rounded-lg text-white/30 hover:text-red-400 hover:bg-white/5 transition-colors" title="Excluir">
            ...
            <>
              ...
              <div>
                {editingDesc ? (
                  <textarea value={desc} onChange={(e) => setDesc(e.target.value)}
                    ...
                    autoFocus rows={4} placeholder="Adicionar descrição / briefing..."
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="relative flex-1">
                  <select value={moveColId} onChange={(e) => handleMove(e.target.value)}
                  ...
                  <div className="mt-2 p-3 bg-[#0a0a0a] border border-[#333] rounded-xl space-y-3">
                    ...
                    <div className="space-y-1 max-h-40 overflow-y-auto">
                      {boardLabels.map((bl) => {
                        ...
                        return (
                          <label key={bl.id} className="flex items-center gap-2 cursor-pointer">
                            <div className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${active ? 'border-transparent' : 'border-[#444]'}`}
