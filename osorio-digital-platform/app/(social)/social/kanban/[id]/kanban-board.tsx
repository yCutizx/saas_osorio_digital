  ...
  return (
    <div ref={setNodeRef} style={style}
      ...
      {card.cover_url && (
        ...
        <img src={card.cover_url} alt="" className="w-full h-28 sm:h-40 object-cover" />
      )}
...
export function KanbanBoard({ board, initialCards, boardMembers, clients, currentUserId }: Props) {
  ...
  // Realtime: qualquer evento dispara router.refresh; props frescas
  // re-hidratam setCards/setColumns via useEffect acima.
  const boardId = board.id

  useEffect(() => {
    const supabase = createBrowserClient()
    const channel  = supabase
      .channel(`social-board-${boardId}`)
      .on(
        'postgres_changes',
      ...
      .subscribe((_status: string, err?: Error) => {
        if (err) console.error(`[Realtime] social-board-${boardId} error:`, err)
      })
