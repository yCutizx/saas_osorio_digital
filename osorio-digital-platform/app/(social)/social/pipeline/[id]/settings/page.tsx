...
export default async function SocialPipelineSettingsPage({ params }: PageProps) {
  ...
  if (!isAdmin && !isCreator && !isMember) redirect('/social/pipeline')
      ...
      <div className="max-w-3xl space-y-10">
        <div>
          <p className="text-xs text-white/30 mb-1">
            ...
            <Link href={`/social/pipeline/${pipeline.id}`} className="hover:text-white/60 transition-colors">{pipeline.name}</Link>
            <span className="mx-1.5">›</span>
