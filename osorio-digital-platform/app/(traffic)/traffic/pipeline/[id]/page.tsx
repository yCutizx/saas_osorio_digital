...
export default async function TrafficPipelineBoardPage({ params }: PageProps) {
  ...
  if (profile.role !== 'admin' && pipeline.created_by !== user.id) {
    const { data: member } = await admin
      ...
      .maybeSingle()
    if (!member) redirect('/traffic/pipeline')
  }
  ...
  const [
    ...
    admin.from('pipeline_members').select('profile:profiles!pipeline_members_profile_id_fkey(id, full_name, email)').eq('pipeline_id', pipelineId),
    admin.from('pipeline_activities').select('lead_id').eq('done', false).lt('scheduled_at', new Date().toISOString()),
  ])
  ...
  const [
    ...
    ? [{ data: [] }, { data: [] }, { data: [] }]
    : await Promise.all([
        admin.from('pipeline_activities').select('id, lead_id, user_id, type, description, scheduled_at, done, created_at').in('lead_id', leadIds).order('created_at', { ascending: false }),
        admin.from('pipeline_lead_attachments').select('id, lead_id, file_name, file_url, file_size, uploaded_by, created_at').in('lead_id', leadIds).order('created_at', { ascending: false }),
        admin.from('pipeline_lead_timeline').select('id, lead_id, user_id, event_type, event_data, created_at, user:profiles!pipeline_lead_timeline_user_id_fkey(full_name, email)').in('lead_id', leadIds).order('created_at', { ascending: false }),
      ])
  ...
  for (const ev of (timelineRaw ?? []) as Array<{
    ...
    timelineByLead[ev.lead_id].push({
      id: ev.id, lead_id: ev.lead_id, user_id: ev.user_id,
      event_type: ev.event_type as LeadTimelineEventType,
      event_data: ev.event_data, created_at: ev.created_at, user: userObj,
    })
  }
  ...
  const leads: Lead[] = ((rawLeads ?? []) as LeadRow[]).map((l) => {
    ...
    const leadTags: PipelineTag[] = tagRows.map((r) => Array.isArray(r.tag) ? r.tag[0] : r.tag).filter((t): t is PipelineTag => !!t)
    const resp = Array.isArray(l.responsible) ? l.responsible[0] ?? null : l.responsible ?? null
    return {
      id: l.id, pipeline_id: l.pipeline_id, name: l.name, company: l.company, email: l.email,
      phone: l.phone, whatsapp: l.whatsapp, role: l.role, source: l.source,
      estimated_value: l.estimated_value, expected_close_date: l.expected_close_date,
      probability: l.probability, stage: l.stage, position: l.position ?? 0,
      notes: l.notes, lost_reason: l.lost_reason, lost_reason_other: l.lost_reason_other,
      responsible_id: l.responsible_id, created_at: l.created_at, updated_at: l.updated_at,
      responsible: resp, tags: leadTags,
    }
  })
  ...
  const canManageSettings = profile.role === 'admin' || pipeline.created_by === user.id || members.some((m) => m.id === user.id)
