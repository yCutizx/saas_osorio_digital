...
export async function POST(request: NextRequest) {
  ...
  if (!webhook) {
    logger.warn('pipeline/inbound: chave inválida ou webhook inativo')
  ...
  if (!name) {
    await admin.from('pipeline_webhook_logs').insert({
      webhook_id: webhook.id, event: 'lead.created', payload: body, status: 'error',
    })
  ...
  if (leadError || !lead) {
    logger.error('pipeline/inbound: erro ao criar lead', { error: leadError?.message })
    await admin.from('pipeline_webhook_logs').insert({
      webhook_id: webhook.id, event: 'lead.created', payload: body, status: 'error',
    })
  ...
  await admin.from('pipeline_webhook_logs').insert({
    webhook_id: webhook.id, event: 'lead.created', payload: body, status: 'success',
  })

  logger.info('pipeline/inbound: lead criado', { id: lead.id, stage: lead.stage })
