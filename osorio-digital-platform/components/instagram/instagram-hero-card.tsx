...
export function InstagramHeroCard({ from, to, username, stats }: Props) {
  ...
  const primaryCards: Array<Card & { tooltip?: string }> = [
    ...
    { label: 'Impressões',        value: fmtLarge(stats.views),         icon: Eye,       color: 'text-purple-400', bg: 'bg-purple-400/10', tooltip: 'Visualizações totais do conteúdo orgânico (posts, reels, stories). Não inclui anúncios pagos.' },
    { label: 'Alcance',           value: fmtLarge(stats.reach),         icon: Target,    color: 'text-green-400',  bg: 'bg-green-400/10',  tooltip: 'Pessoas únicas alcançadas pelo seu conteúdo orgânico + anúncios pagos no período.' },
